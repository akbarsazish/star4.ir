...
class Admin extends Controller
{
    ...
    public function changeDate(Request $request)
    {
        ...
        $eachdays=DB::select("SELECT count(addedDate) as countPeopels,addedDate from(
            ...
            JOIN CRM.dbo.crm_customer_added on Peopels.PSN=crm_customer_added.customer_id
            JOIN CRM.dbo.crm_admin on crm_admin.id=crm_customer_added.admin_id
            WHERE GroupCode=314 and crm_customer_added.admin_id=$adminId)a group by addedDate  order by addedDate desc");
    ...
    public function report(){
        ...
        //     order by lastVisit desc");
                ...
                ->join("CRM.dbo.crm_adminType",'crm_adminType.id','=','crm_admin.adminType')->where('deleted',0)
            ...
            // $newCustomers=DB::select("SELECT PSN,PCode,Name,peopeladdress,CRM.dbo.getCustomerPhoneNumbers(PSN) as PhoneStr,
            // CRM.dbo.getCustomerMantagheh(SnMantagheh) as Mantagheh,
            // CRM.dbo.getLastDateFactor(PSN) as lastFactDate
            // ,CRM.dbo.getAdminName(PSN) as adminName FROM Shop.dbo.Peopels where exists(select * from CRM.dbo.crm_inserted_customers where CONVERT(DATE,addedDate)>='2023-02-05' and customerId=PSN)");  
        ...
        //              'referencialCustomers'=>$referencialCustomers, 'referencialAdmins'=>$referencialAdmins,'returners'=>$returnerAdmins,'inActiverAdmins'=>$inActiverAdmins,'newCustomers'=>$newCustomers]);

        return view ("reports.listReport",['cities'=>$cities, 'admins'=>$admins,'evacuatedAdmins'=>$evacuatedAdmins, 'referencialAdmins'=>$referencialAdmins,'returners'=>$returnerAdmins,'inActiverAdmins'=>$inActiverAdmins]);
    }
    ...
    JOIN(select COUNT(id) as countCycle,factorId as factorSn from CRM.dbo.crm_alarm group by factorId)b on b.factorSn=factorId
                                WHERE  GroupCode IN ( ".implode(",",Session::get("groups")).") and CompanyNo=5  and alarmDate<='".$todayDate."' and state=0 and not exists(select customerId from CRM.dbo.crm_inactiveCustomer where customerId=PSN and crm_inactiveCustomer.state=1) ORDER BY alarmDate desc" );
        foreach ($customers as $customer) {
    ...
    public function getAlarmsHistory(Request $request)
    {
        ...
        LEFT JOIN (select name poshtibanName,lastName as poshtibanLastName,customer_id,admin_id as adminSn from CRM.dbo.crm_customer_added join CRM.dbo.crm_admin on admin_id=crm_admin.id where returnState=0)n on k.PSN=n.customer_id
        JOIN(select COUNT(id) as countCycle,factorId as factorSn from CRM.dbo.crm_alarm group by factorId)b on b.factorSn=factorId
                                    WHERE  GroupCode IN ( ".implode(",",Session::get("groups")).") and CompanyNo=5  and alarmDate='".$yesterday."' and state=0 and not exists(select customerId from CRM.dbo.crm_inactiveCustomer where customerId=PSN)");
            foreach ($customers as $customer) {
            $customer->assignedDays=\Morilog\Jalali\CalendarUtils::createCarbonFromFormat('Y/m/d',trim($customer->alarmDate))->diffInDays($customer->TimeStamp);
...
$customer->PassedDays=\Morilog\Jalali\CalendarUtils::createCarbonFromFormat('Y/m/d',trim($customer->alarmDate))->diffInDays(Carbon::now());
    ...
    public function adminDashboard(Request $request)
    {
        ...
        if($admin->emptyState==0){
            ...
            return Response::json([$admins,$info,$history,$customers,'GOOD']);
        }else{
...
// در صورتیکه راننده باشد
    ...
    if($personType->adminType==7){
        ...
        //ثبت تاریخچه
        DB::table("CRM.dbo.crm_adminHistory")->insert(['adminId'=>$adminId,'countPeople'=>$allActiveCustomerCount,'countFactor'=>$countAllFactor,'countBuyPeople'=>$boughtPeopelsCount,'factorAllMoney'=>$sumAllFactor
...
//امتیازات این ماه بعد از تخلیه
    ...
    public function filteralarms(Request $request)
    {
        ...
        if(strlen($request->get("secondDateAlarm"))>3){
            $firstDateAlarm=$request->get("secondDateAlarm");
        }
        ...
        if($alarmStat==0){
                $customers=DB::select("SELECT * FROM (
                            ...
                            LEFT JOIN (select name poshtibanName,lastName as poshtibanLastName,customer_id,admin_id as adminSn from CRM.dbo.crm_customer_added join CRM.dbo.crm_admin on admin_id=crm_admin.id where returnState=0)n on m.CustomerSn=n.customer_id
                            JOIN(select COUNT(id) as countCycle,factorId as factorSn from CRM.dbo.crm_alarm group by factorId)b on b.factorSn=factorId
                                WHERE  GroupCode IN ( ".implode(",",Session::get("groups")).") AND CompanyNo=5  AND alarmDate<='".$todayDate."' and state=0 and not exists(select customerId from CRM.dbo.crm_inactiveCustomer where customerId=PSN and crm_inactiveCustomer.state=1)" );
                foreach ($customers as $customer) {
                $customer->assignedDays=\Morilog\Jalali\CalendarUtils::createCarbonFromFormat('Y/m/d',trim($customer->alarmDate))->diffInDays($customer->TimeStamp);
