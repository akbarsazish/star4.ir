...
function returnedCustomerStuff(element) {
    ...
    $(".enableBtn").prop("disabled", false);
    $(".enableBtn").val(adminID);
    $("#customerSnLogin").val(adminID);
}
...
$("#searchAllName").on("keyup", () => {
    ...
    if ($(".reportRadio:checked").val() == "newCustomers") {
        $.ajax({
            method: "get",
            url: baseUrl + "/searchNewCustomerByName",
            data: {
                _token: "{{ csrf_token() }}",
                searchTerm: searchTerm,
                SnMantagheh: snMantagheh
        ...
        success: function (respond) {
            ...
            respond.forEach((element, index) => {
                ...
                $("#historyListBody").append(`
                    <tr onclick="setUpDownHistoryStuff(this,` + element.historyId + `)">
                        <td>`+ (index + 1) + `</td>
