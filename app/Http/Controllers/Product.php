...
class Product extends Controller
{
    ...
    public function salesReport(){
        ...
        $poshtibans=DB::select("SELECT * FROM CRM.dbo.crm_admin a where exists (select * from CRM.dbo.crm_customer_added where returnState=0 and admin_id=a.id)");
        return view("reports.salesReport",['regions'=>$regions,'poshtibans'=>$poshtibans]);
    }
    ...
    public function getSalesReportInfo(Request $request)
    {
        ...
        $reportInfo=DB::select("SELECT * FROM (select PSN,PCode,Peopels.CompanyNo,Name,sumAllMoney,CountFactor,ISNULL(adminName,'') as adminName,adminId,CRM.dbo.getCustomerMantagheh(SnMantagheh) as MantaghehName from Shop.dbo.Peopels 
       left Join(SELECT SUM(NetPriceHDS)/10 AS sumAllMoney,CustomerSn FROM Shop.dbo.FactorHDS  WHERE FactType=3 and FactDate>'$firstDate' and FactDate<='$secondDate' GROUP BY CustomerSn)a on a.CustomerSn=PSN 
        left Join (
        SELECT Count(NetPriceHDS) AS CountFactor,CustomerSn FROM Shop.dbo.FactorHDS  WHERE FactType=3 and FactDate>'$firstDate' and FactDate<='$secondDate' GROUP BY CustomerSn)b on b.CustomerSn=PSN 
        ...
        where (PCode Like'%$nameCode%' or Name LIKE '%$nameCode%') AND CompanyNo=5  and MantaghehName like'%$mantaghehName%' and adminName like '%$adminName%' order by sumAllMoney desc
        ");
